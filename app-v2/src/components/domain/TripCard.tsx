...
const TripCard: React.FC<Props> = ({ item, onPress }) => {
  ...
  const preco = MaskService.toMask('money', String(item.preco), { unit: 'R$ ', separator: ',', delimiter: '.' });
    ...
    <TouchableOpacity style={styles.card} onPress={onPress} activeOpacity={0.85}>
      <View style={styles.header}>
        <View style={styles.route}>
          ...
          <Ionicons name="arrow-forward" size={16} color={Colors.primary} style={{ marginHorizontal: 6 }} />
...
const styles = StyleSheet.create({
  ...
  price:    { ...Typography.heading3, color: Colors.primary },
